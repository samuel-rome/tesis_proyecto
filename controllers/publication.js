...
const feed = async (req, res) => {
  ...
  try {
    ...
    let result = await Promise.all(myFollows.following.map(async id => {
        const data = await Publication.find({ user: id });
        return data;
    }));

    result = result.filter(e => e.length > 0).flat()
    
    const completResult = await Promise.all(result.map(async entity => {
        const data = await User.findOne({ _id: entity.user });
        return   {
          user: data,
          text: entity.text,
